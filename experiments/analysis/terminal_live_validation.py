...
def _eligible_attempts(root,results_dir):
    output=[]
...
def analyze(root:Path,results_dir="experiment_results",tolerance=1e-9):
    rr=root/results_dir;attempts=_eligible_attempts(root,rr);profile_json=read_json(root/"competition_code/shadow/velocity_profile.json");profile={"target_speed_kmh":[x["target_speed_kmh"] for x in profile_json["points"]],"ds_m":[x["ds_m"] for x in profile_json["points"]]}
    ...
    for item,rows in attempts:
        ...
        for index,row in enumerate(rows):
            ...
            if live["status"]=="RELEASE":
                terminal=replay["terminal_profile_index"];future=None
    ...
    result["recommendation"]=("A_READY_FOR_APPLIED_MICRO_AB_DESIGN" if len(attempts)>=10 and not mismatches and not invariant_failures and not result["retrospective_release_audit"].get("C_winner_braking_terminal_exceeded",0) else "B_COLLECT_MORE_SHADOW_DATA" if not mismatches and not invariant_failures else "C_RETURN_TO_OFFLINE_MODELING")
